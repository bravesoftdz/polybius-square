  ...
  TForm39 = class(TForm)
    ...
    procedure Image1MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
		...
		procedure EditxxClick(Sender: TObject);
		procedure EditxxOnMouseEnter(Sender: tobject);
		procedure EditxxKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
...
form_activated:=false;
		...
		{#region filling array of edits}             {$REGION}
...
for I := 0 to 255 do begin
	edits_array[i].Tag				:=	i;
	edits_array[i].ShowHint		:= 	true;
	edits_array[i].OnChange 	:=  EditsOnChange;
	edits_array[i].OnClick	 	:=	EditxxClick;
	edits_array[i].OnKeyDown	:=	EditxxKeyDown;
	edits_array[i].OnMouseEnter:= EditxxOnMouseEnter;
...
procedure TForm39.Label1Click(Sender: TObject);
var t0: cardinal;
		keyptr2:Pkey2;
begin
keyptr2:=@table2;
t0 := gettickcount();
generate_random_key(0, 4194304, 0, 255, true, keyptr2);
t0 := gettickcount() - t0;
showmessage('generate_random_key('+inttostr(length(table2))+'): '+inttostr(t0)+'ms');
end;
...
if Sender is TEdit then begin
  ...
  form39.lock;                                     //скажем нет рекурсии!
  ...
  if length(table2) = 0 then begin
		maskcollector;
		keyPtr := @table;
		intersections := encoder.volidate(keyPtr) end
	else begin
		...
		if edits_array[ID].Text <> '' then
			...
			table2[i2 - 1][ID] := 0;
		keyPtr := @table2[i2-1];
		intersections := encoder.volidate(keyPtr);
	end;
	for I := 0 to 255 do
		edits_array[i].color := clWindow;
	for I := 1 to length(intersections) do
...
procedure TForm39.EditxxOnMouseEnter(Sender: tobject);
var i, ID:byte;
		sum: int64;
begin
if Sender is TEdit then begin
	...
	if sum > 0 then
		if edits_array[ID].Text = '' then
			edits_array[ID].Hint := inttohex(0,2) + ' ('+
				floattostrf(frequence[0]/sum*100, fffixed,5,4)+'%)'
		else
			edits_array[ID].Hint := inttohex(ord(edits_array[ID].Text[1]),2) + ' ('+
				floattostrf(frequence[ord(edits_array[ID].Text[1])]/sum*100, fffixed,5,4)+'%)'
	else edits_array[ID].Hint := '';
end;
end;

procedure TForm39.editxxKeyDown(Sender: TObject; var Key: Word;
	Shift: TShiftState);
var ID: byte;
begin
ID := (Sender as TEdit).Tag;
case key of
	VK_UP:		if id > $10 then dec(id, $10) else inc(id, $F0);
	VK_DOWN: 	if id < $F0 then inc(id, $10) else dec(id, $F0);
	VK_LEFT: 	if id > 	0 then dec(id,   1) else id := $FF;
	VK_RIGHT:	if id < $FF then inc(id,   1) else id :=   0;
end;

if length(table2) > 0 then
case key of
	VK_NEXT:  trackbar1.Perform(WM_KEYDOWN,VK_RIGHT,0);
	VK_PRIOR: trackbar1.Perform(WM_KEYDOWN,VK_LEFT, 0);
end;

edits_array[id].SetFocus;
end;

procedure TForm39.EditxxClick(Sender: TObject);
begin
...
begin
	...
	for I := 0 to 255 do
		edits_array[i].OnChange := nil;
end;

procedure TForm39.MaskEdit1Click(Sender: TObject);
begin
	MaskEdit1.SelectAll;
end;
...
if savedialog1.execute(handle) then
	case savedialog1.FilterIndex of
		...
		5: image1.Picture.SaveToFile(savedialog1.FileName);
	end;
end;
																								 {$ENDREGION}
																								 {$ENDREGION}
...
begin
  for I := 0 to 255 do begin
	...
	maskcollector;
	keyPtr := @table;
	intersections := volidate(keyPtr);
	for I := 1 to length(intersections) do
		edits_array[ord(intersections[i])].Color:=$7F7FFF;
end;

procedure TForm39.maskcollector;
var i:byte;
begin
	for I := 0 to 255 do begin
