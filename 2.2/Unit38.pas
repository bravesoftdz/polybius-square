...
resourcestring
  ...
  SFileCorrupted				=	'Файл повреждён';
	...
	SKeyInvar							=	'Ключ инвариантен к данному преобразованию.';
...
if inifile.ReadBool('Encoding',	'forced_text',true) then begin
	N29.Checked := true;
	N33.Checked := true;
end;
...
begin
	...
	else Threads.run(dt_MD5);
	if not binary_mode then
		ptr^ := '';
end;
...
if binary_mode then begin
	if length(binary_data) >= Thresold then
	begin
		Threads.run(dt_Frequency_analysis);
		EXIT;
	end else begin
