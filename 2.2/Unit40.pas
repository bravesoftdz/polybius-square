...
if not datathread.suspended then try
	if (DataThread.Todo = dt_encode_dyn) or (DataThread.Todo = dt_decode_dyn) or
			((DataThread.Todo = dt_batch) and (batchForm.CheckBox11.Checked)) then begin
		progress := DataThread.progress;
		Label1.Caption := inttostr		(Round(progress * 100)) +' %';
		if  (DataThread.Todo = dt_batch) and (DataThread._FileSize < $100000) then
			label2.Caption := SEstimatedTime + 'N/A'
		else
			label2.Caption := SEstimatedTime + inttostr(Round((1 - progress) / (DataThread.speed)))+' s';
	...
	if DataThread.Todo <> dt_large_file then
		...
		label2.Caption := SEstimatedTime + inttostr(Round((1 - progress) / (DataThread.speed+0.01)))+' s';
...
case dataThread.Todo of
	dt_encode_dyn, dt_decode_dyn: SetLength(Table2, 0);
end;
dataThread.Free;
Close;
